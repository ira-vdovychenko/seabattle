class BattlefieldView extends Battlefield {
    root = null;//весь додаток
    table = null;//поле 10*10
    dock = null;//div в якому будуть зберігатись усі кораблі
    polygon = null;// div в якому будуть зберігатись усі вистріли
    showShips = true;//властивість яка контролює чи показувати кораблі візуально
...
// стандартний спосіб створення матриці
        for (let y=0; y<10; y++) {
            const row = [];
            const tr = document.createElement('tr'); //<tr> (table row - рядок таблиці):
            tr.classList.add('battlefield-row');
            tr.dataset.y = y;

            for (let x=0; x<10; x++) {
                const td = document.createElement('td');//<td> (table data cell - комірка даних таблиці):
    ...
    removeShot (shot) {
        ...
        if (Array.prototype.includes.call(this.polygon.children, shot.div)) {//.call() для виклику методу Array.prototype.includes 
