//стартове розположення кораблів
const shipDatas = [
...
class PreparationScene extends Scene {
    ...
    start() {   
        ...
        const randomizeButton = document.querySelector('[data-action="randomize"]');//кнопка розставити кораблі випадково
        const manuallyButton = document.querySelector('[data-action="manually"]');//кнопка розставити кораблі вручну
    ...
    update() {
        ...
        if (!mouse.left && this.draggedShip) {
            ...
            //опрацювання якщо корабель ставимо за межі комірок(ігр.поля)              
            } else {
                player.removeShip(ship);
                player.addShip(ship);
            }
        ...
        //функціонал, якщо всі кораблі розставлені - включаєм 3 верхні кнопки
        if (player.complete) {
